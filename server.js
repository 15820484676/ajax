...
var server = http.createServer(function (request, response) {
  ...
  if (path === "/index.html") {
    ...
    const result = array.map((item) => `<li>${item}</li>`).join("");
    string = string.replace("{{page1}}", `<ul id="flute">${result}</ul>`);
    ...
    response.end();
  } else if (path === "/style.css" || path === "/test.css") {
    response.statusCode = 200;
    response.setHeader("Content-Type", "text/css;charset=utf-8");
    response.write(fs.readFileSync("public/test.css"));
    response.end();
